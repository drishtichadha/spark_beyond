...
class Problem(BaseModel):
    target: str
    type: ProblemType
    desired_result: Union[str, int] = None
...
class SchemaChecks:
    ...
    def datetime_checks(self):
        if (self.problem.date_column is not None) and (self.problem.date_column not in self.dataframe.columns):
            raise ValueError(f"Date column {self.problem.date_column} not found.")
        if not self.get_col_type(self.problem.target) not in getattr(ColumnTypes, "datetime").value:
            raise ValueError(f'Date column {self.problem.date_column} must have a temporal dtype (date, time or timestamp), found {self.get_col_type(self.problem.target)}')
